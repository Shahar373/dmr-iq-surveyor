...
`--campaign <id>` tags a run with the collection round it belongs to -- on `survey run`, `survey capture`, `live stop` and `web serve` -- and `scripts/campaign_digest.py --campaign <id>` narrows the digest to it. Left unset a run is unassigned, which is what every run recorded before campaigns existed is; none was backfilled.
...
- **Gain drift.** Measurements from a stop recorded at a gain other than the campaign's are
  ...
  every run -- so stops sharing one site profile all report the same value. Each run now also
  records its own receiver state in `survey_runs.hardware_json`, keeping what the radio reported
  back apart from what it was asked for, and `scripts/campaign_digest.py` shows every gain with
  that label. The solver still reads the old value.
